...
// Function to create and animate card play effects
function createCardPlayAnimation(card) {
  ...
  // Create the card element for animation
  const cardElem = createCardElement(card, true);
  
  // Style for animation
  cardElem.style.position = 'absolute';
  cardElem.style.top = '70%';
  cardElem.style.left = '50%';
  cardElem.style.transform = 'translate(-50%, -50%) scale(0.5) rotate(5deg)';
  cardElem.style.boxShadow = '0 5px 20px rgba(0, 0, 0, 0.5)';
  cardElem.style.transition = 'all 0.8s cubic-bezier(0.165, 0.84, 0.44, 1)';
  
  // Add sparkle effects for special cards
  if (card.value === 'Wild' || card.value === 'Wild Draw 4' || 
      card.value === 'Skip' || card.value === 'Reverse' || card.value === 'Draw 2') {
    // Add sparkles or stars around the card
    for (let i = 0; i < 10; i++) {
      ...
      sparkle.style.top = `calc(70% + ${Math.sin(angle) * distance}px)`;
  ...
  animContainer.appendChild(cardElem);
  document.body.appendChild(animContainer);
  
  // Start animation after a tiny delay for the DOM
  setTimeout(() => {
    cardElem.style.top = '40%';
    cardElem.style.transform = 'translate(-50%, -50%) scale(1.2) rotate(-5deg)';
    
    // Add a woosh sound effect
    const audio = new Audio();
    audio.volume = 0.3;
    audio.src = 'data:audio/mpeg;base64,';
    audio.play();
...
// Add event listener for the new card played event
window.addEventListener('cardPlayed', (event) => {
  // Get card information from the event
  const { card, playerIndex, isAI } = event.detail;
  
  // Create animation only for certain cards (special cards) or for human player
  if (!isAI || 
      card.value === 'Wild' || 
      card.value === 'Wild Draw 4' || 
      card.value === 'Skip' || 
      card.value === 'Reverse' || 
      card.value === 'Draw 2') {
    createCardPlayAnimation(card);
  }
  
  // Add a pulse effect to the discard pile
  const discardPile = document.getElementById('discard-pile');
  if (discardPile) {
    // Add a class for the animation if it doesn't exist yet
    if (!document.getElementById('discard-pulse-animation')) {
      const style = document.createElement('style');
      style.id = 'discard-pulse-animation';
      style.textContent = `
        @keyframes discard-pulse {
          0% { transform: scale(1); box-shadow: 0 0 10px rgba(255, 255, 255, 0.5); }
          50% { transform: scale(1.1); box-shadow: 0 0 25px rgba(255, 255, 255, 0.8); }
          100% { transform: scale(1); box-shadow: 0 0 10px rgba(255, 255, 255, 0.5); }
        }
        .pulse-effect {
          animation: discard-pulse 0.6s ease-in-out;
        }
      `;
      document.head.appendChild(style);
    }
    ...
    discardPile.classList.add('pulse-effect');
    
    // Remove the class after animation finishes to allow it to be triggered again
    setTimeout(() => {
      discardPile.classList.remove('pulse-effect');
    }, 600);
  }
});
